...
public final class ManualNodeMiningHandler {
    ...
    public static boolean handle(Level level, Player player, BlockPos pos) {
        ...
        if (now < PlayerScanState.mineCooldown((net.minecraft.server.level.ServerPlayer) player)) {
            return true;
        }
        ...
        PlayerScanState.mineCooldown((net.minecraft.server.level.ServerPlayer) player, now + cooldown);
